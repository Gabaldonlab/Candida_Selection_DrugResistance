...
# It takes a df with diploid SNPs (already filtered for not having positions with low coverage or INDELs) and it runs iqtree to generate a tree from an alignment where the heterozygous SNPs are taken (or not) randomly.
...
# get the samples
sorted_samples = sorted(pd.read_csv(opt.samples_file, sep="\t").sampleID.apply(str))
...
if len(chroms_without_SNPs)>0: print("WARNING: There are some chroms without SNPs: %s. These are their lengths:\n%s"%(chroms_without_SNPs, "\n".join(["%s: %i"%(c, chr_to_len[c]) for c in chroms_without_SNPs])))
...
"""
# slow way to generate multifasta
